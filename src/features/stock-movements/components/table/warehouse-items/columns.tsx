...
export const warehouseItemsColumnsDefs: ColumnDef<WarehouseItem>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div>
          <div className='font-medium'>{available.toString()} шт.</div>
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return <div className={isLow ? 'text-red-600' : ''}>{row.getValue('minimumQuantity')}</div>
    },
  },
  {
    id: 'price',
  ...
  },
  {
    ...
    cell: ({ row }) => <div>{new Date(row.getValue('updatedAt')).toLocaleDateString('ru-RU')}</div>,
  },
  {
    id: 'actions',
    cell: ({ row }) => {
