...
import { useAddSpecProduct, useAddSpecService, useDeleteSpecProduct, useDeleteSpecService } from '@/features/service-specifications/api/mutations'
...
const ServicesTab = ({ specId }: { specId: number }) => {
  ...
  const handleAdd = () => {
    if (!pending) return
    addService(
      { serviceId: pending.service.id, defaultDuration: pending.duration, discountPercent: undefined },
  ...
  return (
    <Sheet open={!!specId} onOpenChange={(open) => !open && onOpenChange(false)}>
      <SheetContent className='sm:max-w-lg'>
        <SheetHeader>
          <SheetTitle>{spec?.name ?? 'Спецификация'}</SheetTitle>
        </SheetHeader>
        ...
        {isLoading ? (
          ...
          <Tabs defaultValue='services' className='mt-4'>
            <TabsList className='w-full'>
              <TabsTrigger value='services' className='flex-1'>Услуги</TabsTrigger>
