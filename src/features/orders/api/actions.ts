...
export const getOrderServicesById = async (orderId: number) => {
  const response = await apiClient.get<OrderService[]>(
    `${ORDERS_URL}/${orderId}${SERVICES_URL}`
  );
  return response.data;
};
...
export const addMechanicOrderServiceFn = async (
...
export const deleteMechanicOrderServiceFn = async (
  orderServiceId: number,
  mechanicId: number
) => {
...
export const editOrderServiceFn = async (
  orderServiceId: number,
  data: Partial<OrderService>
) => {
  const response = await apiClient.patch(
    `${ORDERS_URL}${SERVICES_URL}/${orderServiceId}`,
    data
  );
  return response.data;
};
...
export const getClientsOrders = async (clientId: number) => {
  const response = await apiClient.get<Order[]>(
    `${ORDERS_URL}${CLIENT_ORDERS}/${clientId}`
  );
  return response.data;
};
