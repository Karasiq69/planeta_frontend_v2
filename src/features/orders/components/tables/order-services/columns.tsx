...
export const ServicesColumnDefs: ColumnDef<OrderService>[] = [
  {
    ...
    cell: ({ row }) => (
      <div>
        <p className='m-0 text-muted-foreground'>{String(row.original.id).padStart(8, '0')}</p>
      </div>
    ),
  },
  {
    ...
    cell: ({ row }) => <p className='font-medium m-0 w-auto'>{row.original.service?.name}</p>,
  },
  ...
  },
  {
    ...
    cell: ({ row }) => <div className="">1 шт</div>,
  },
  {
    accessorKey: 'totals',
        ...
        <div className='text-nowrap'>
          {orderService.discountPercent > 0 && (
            <span className='text-muted-foreground text-xs mr-2'>
              {Math.round(
                (orderService.defaultDuration / 60) * orderService.appliedRate
...

  {
    ...
    cell: ({ row }) => {
      return <OrderServicesTableActions rowInstance={row} />
    },
  },
]
