...
import type { DateSelectArg, EventChangeArg, EventClickArg, EventContentArg } from '@fullcalendar/core'
  ...
  const renderEventContent = (eventInfo: EventContentArg) => {
    ...
    return (
      <div className='w-full overflow-hidden '>
        <div className='bg-background/50 rounded-t-sm p-1 text-xs'>
          {getTimeFromDate(event.start)} - {getTimeFromDate(event.end)}
        </div>
        <div className='font-semibold'>{event.title}</div>

        {orderId && <div className='text-xs font-medium'>Заказ №{orderId}</div>}

        {description && <div className='text-xs mt-1 line-clamp-2'>{description}</div>}
      </div>
    )
  }
              ...
              <DialogContent className="max-w-3xl">
                ...
                {currentAppointment && (
                  <AppointmentForm
                    ...
                    onUpdate={(eventId) => (data: AppointmentInput) =>
                      handleUpdateAppointment(eventId, data)
                    }
