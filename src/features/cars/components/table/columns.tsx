...
export const vehiclesColumns: ColumnDef<ICar>[] = [
  {
    accessorKey: 'brand',
    header: ({ column }) => <DataTableColumnHeader column={column} title='Марка' />,
    cell: ({ row }) => {
      ...
      return (
        <div className='flex  items-center gap-3   text-nowrap'>
          <Avatar className=" h-7 w-7  ">
            <AvatarImage className={' '} src={logoSrc} alt={car.brand?.name || 'Car brand'} />
            <AvatarFallback delayMs={900}>{car.brand?.name?.charAt(0) || 'B'}</AvatarFallback>
          </Avatar>
          <div>
  ...
  },
  {
    accessorKey: 'vin',
    header: 'VIN',
    cell: ({ row }) => <div className='capitalize text-sm'>{row.getValue('vin')}</div>,
  },
  {
  ...
  //
  {
    ...
    cell: ({ row }) => {
      ...
      return <LicencePlateBadge licence_plate={licencePlate || ''} />
    },
    enableSorting: false,
    enableHiding: true,
    enableGlobalFilter: true,
