...
import type { ReceiptDocument } from '@/features/inventory-documents/types'
...
export const inventoryDocumentReceiptColumns: ColumnDef<ReceiptDocument>[] = [
  ...
  },
  {
    ...
    header: ({ column }) => <DataTableColumnHeader column={column} title='Номер вх. документа' />,
    cell: ({ row }) => {
  ...
  },
  {
    accessorKey: 'user',
    meta: 'Ответственный',
    header: ({ column }) => <DataTableColumnHeader column={column} title='Ответственный' />,
    cell: ({ row }) => {
