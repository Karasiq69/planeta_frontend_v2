...
import {InventoryDocumentType, ReceiptDocument, UpdateReceiptDocumentDto} from "@/features/inventory-documents/types";
...
const ReceiptDocumentForm = ({onSubmit, documentData: doc}: Props) => {
    const defaultValues = useMemo<UpdateReceiptDocumentDto>(() => {
        return {
            ...
            incomingDate: doc?.receiptDocument?.incomingDate ? new Date(doc?.receiptDocument?.incomingDate) : undefined,
            ...
            <form id="receiptDocumentForm" onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6 md:gap-x-16 max-w-6xl">
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center gap-3">
                        <Label htmlFor="document-type" className="lg:w-32 text-muted-foreground">Тип:</Label>
                    ...
                    <div className="flex flex-col lg:flex-row md:items-start gap-3">
                        <Label htmlFor="document-number"
                               className="lg:w-32 text-muted-foreground md:pt-2">Номер:</Label>
                        <div
                            className="w-full grid grid-cols-[1fr_auto_1fr] md:grid-cols-[1fr_auto_1fr] items-center gap-2">
                            <FormField
                                disabled
                                control={form.control}
                                name="number"
                            ...
                            />
                            <Label htmlFor="document-date" className="text-center">от</Label>
                            <FormField
                                control={form.control}
                                name="date"
                                render={({field}) => (
                    ...
                    {/* Входящий номер */}
                    <div className="flex flex-col lg:flex-row md:items-start gap-3">
                        <Label htmlFor="incoming-number" className="lg:w-32 text-muted-foreground md:pt-2">Вх.
                            номер:</Label>
                        <div
                            className="w-full grid grid-cols-[1fr_auto_1fr] md:grid-cols-[1fr_auto_1fr] items-center gap-2">
                            <FormField
                            ...
                            />
                            <Label htmlFor="incoming-date" className="text-center">от</Label>
                            <FormField
                                control={form.control}
                                name="incomingDate"
                                render={({field}) => (
                    ...
                    {/* Склад */}
                    <div className="flex flex-col lg:flex-row lg:items-center gap-3">
                        <Label htmlFor="warehouse-select" className="lg:w-32 text-muted-foreground">Склад:</Label>
                        <div className="w-full">
                            <FormField
                                control={form.control}
                                name="warehouseId"
                    ...
                    <div className="col-span-1 flex flex-col lg:flex-row lg:items-center gap-3">
                        <Label htmlFor="note-field" className="lg:w-32 text-muted-foreground">Примечание:</Label>
                        <div className="w-full">
                            <FormField
                                control={form.control}
                                name="note"
