...
export const getReceiptDocuments = async (
  params: ReceiptDocumentsQueryParams
): Promise<ListResponse<ReceiptDocument>> => {
...
export const updateReceiptDocument = async (
  id: number,
  data: UpdateReceiptDocumentDto
): Promise<ReceiptDocument> => {
...
export const getReceiptDocumentItems = async (
  documentId: number
): Promise<ReceiptDocumentItem[]> => {
  const response = await apiClient.get<ReceiptDocumentItem[]>(
    `${RECEIPT_DOCUMENTS_URL}/${documentId}/items`
  )
  return response.data
}

// Добавление товара в приходной документ
export const addReceiptDocumentItem = async (
  documentId: number,
  data: ReceiptItemDto
): Promise<ReceiptDocumentItem> => {
  const response = await apiClient.post<ReceiptDocumentItem>(
    `${RECEIPT_DOCUMENTS_URL}/${documentId}/items`,
    data
  )
  return response.data
...
export const updateReceiptDocumentItem = async (
  documentId: number,
  itemId: number,
  data: Partial<ReceiptItemDto>
): Promise<ReceiptDocumentItem> => {
