...
import {TransferDocument, TransferDocumentsQueryParams, TransferItemDto} from "@/features/inventory-documents/transfer/types";
...
export const getTransferDocumentsFn = async (params: TransferDocumentsQueryParams): Promise<ListResponse<TransferDocument>> => {
    const response = await apiClient.get<ListResponse<TransferDocument>>(TRANSFER_DOCUMENTS_URL, {params});
    return response.data;
};

// Создание нового приходного документа
export const createTransferDocumentFn = async (data: CreateTransferDocumentDto): Promise<TransferDocument> => {
    const response = await apiClient.post<TransferDocument>(TRANSFER_DOCUMENTS_URL, data);
    return response.data;
};

// Обновление данных приходного документа
export const updateTransferDocumentFn = async (id: number, data: UpdateTransferDocumentDto): Promise<TransferDocument> => {
...
export const addTransferDocumentItem = async (documentId: number, data: TransferItemDto): Promise<TransferDocument> => {
    const response = await apiClient.post<TransferDocument>(`${TRANSFER_DOCUMENTS_URL}/${documentId}/items`, data);
    return response.data;
};

// Обновление товара в документе перемещения
export const updateTransferDocumentItem = async (documentId: number, itemId: number, data: Partial<TransferItemDto>): Promise<TransferDocument> => {
    const response = await apiClient.patch<TransferDocument>(`${TRANSFER_DOCUMENTS_URL}/${documentId}/items/${itemId}`, data);
    return response.data;
};
...
export const completeTransferDocument = async (id: number): Promise<TransferDocument> => {
    const response = await apiClient.post<TransferDocument>(`${TRANSFER_DOCUMENTS_URL}/${id}/complete`);
    return response.data;
};
