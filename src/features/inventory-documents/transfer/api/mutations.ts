...
import type { TransferItemDto } from '@/features/inventory-documents/transfer/types'
...
export const useUpdateTransferDocumentItem = (documentId: number, itemId: number) => {
  ...
  return useMutation({
    mutationFn: (data: Partial<TransferItemDto>) =>
      updateTransferDocumentItem(documentId, itemId, data),
    onSuccess: () => {
